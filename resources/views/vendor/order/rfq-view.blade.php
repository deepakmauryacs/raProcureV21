@extends('admin.layouts.app_second',['title'=>'Help and Support','sub_title'=>''])
@section('css')
<style>
ul.add-FQR-list {
    display: flex;
...
<div class="breadcrumb-header">
    <div class="container-fluid">
        <h5 class="breadcrumb-line">
            ...
            <a href="{{ route('vendor.rfq_order.index') }}"> ->Orders Confirmed</a>
            <a href="{{ route('vendor.rfq_order.index') }}"> ->View Order</a>
        </h5>
    </div>
</div>
@endsection
            ...
            <div class="card-header bg-transparent">
                <div class="col-md-12 d-flex justify-content-between">
                    ...
                    <div class="rfq-filter-button">
                        <a href="javascript:void(0);" onclick="window.open(`{{ route('vendor.rfq_order.print', $order->id) }}`, '_blank', 'width=800,height=600,resizable=yes,scrollbars=yes'); return false;"
                            class="btn-rfq btn-rfq-white float-right mr-3   py-2 px-2"><i
                                class="bi bi-download"></i>Download </a>
                        <a href="{{ route('vendor.rfq_order.index') }}"
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="text-center" width="60">MRP(₹)</th>
                            <th scope="col" class="text-center" width="60">DISCOUNT(%)</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{$value->frq_variant->specification}}</td>
                            <td class="text-center">{{$value->frq_variant->size}}</td>
                            <td class="text-center">{{$value->order_quantity}}</td>
                            <td class="text-center">{{$value->frq_variant->uoms->uom_name}}</td>
                            <td class="text-center">{{$value->order_mrp}}</td>
                            <td class="text-center">{{$value->order_discount}}</td>
                            <td class="text-center">{{$value->order_price}}</td>
