    ...
    <tbody>
        ...
        @forelse ($results as $key => $result)
            <tr>
                ...
                <td>{{ $result->latestPlan->plan_name?? ''}}</td>
                ...
                <td>{{ $result->latestPlan->start_date ?? ''}}</td>
                <td>{{ $result->latestPlan->subscription_period ?? ''}}</td>
                <td>{{ $result->latestPlan->next_renewal_date ?? '' }}</td>
                <td>₹{{ $result->latestPlan->final_amount ?? '' }}</td>
                <td>
                    <select class="form-select" id="assigned_manager" name="assigned_manager" onchange="assignedManager('{{ $result->id }}',this.value);">
                ...
                @if(!empty($result->latestPlan)&&$result->latestPlan->final_amount > 0)
                    <span>
                        <a href="{{ route('admin.accounts.vendor.plan.view', $result->latestPlan->id) }}" class="btn-rfq btn-rfq-primary btn-sm">View Current</a>
                    </span>
                    ...
                    <div class="row plan-row">
                        <div class="col-md-4">
                            <select class="form-select" id="extend_plan_month_{{ $result->id }}">
                                <option value=""> </option>
